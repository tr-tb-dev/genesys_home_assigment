...
async function fetchNewPostIds(): Promise<HNItemId[]> {
  const response = await fetch(`${API_BASE_URL}/newstories.json`);
  if (!response.ok) {
    throw new Error(`Failed to fetch new post IDs: ${response.status} ${response.statusText}`);
  }
...
async function fetchNewPosts(page: number): Promise<{ posts: HNItem[]; totalPages: number }> {
  const allPostIds = await fetchNewPostIds();
...
function NewPostsTanStack() {
  const [searchParams, setSearchParams] = useSearchParams();
  const viewMode = useSelector(selectViewMode);
  const pageFromUrl = getPageFromUrl(searchParams);

  const { data, isLoading, error } = useQuery({
    queryKey: ['newPosts', pageFromUrl],
    queryFn: () => fetchNewPosts(pageFromUrl),
    staleTime: 60000,
    retry: 2,
  });
...
export default NewPostsTanStack;
